...
public class FormEntryActivity extends Activity implements AnimationListener, FormLoaderListener,
    ...
    private TreeElement getChildByNameRecursive(TreeElement root, String name)
    {
        Vector v = new Vector();
        v.add(root);
        while (v.size() > 0) {
        	TreeElement el = (TreeElement)v.remove(0);
        	if (el == null) continue;
        	if (el.getName().equals(name)) {
        		return el;
        	}
        	for (int i = 0; i < el.getNumChildren(); i++) {
        		TreeElement el2 = el.getChildAt(i);
        		v.add(el2);
        	}
        }
        return null;
    ...
    private View createView(int event) {
        ...
        switch (event) {
            ...
            case FormEntryController.EVENT_END_OF_FORM:
                ...
                FormInstance forminstance = mFormController.getInstance();
                TreeElement root = forminstance.getRoot();
                TreeElement section_basic = getChildByNameRecursive(root, "section_basic");
                String label = mFormController.getFormTitle();
                if (section_basic != null) {
                	TreeElement house_nr = getChildByNameRecursive(section_basic, "house_nr");
                	TreeElement street_address = getChildByNameRecursive(section_basic, "street_address");
                	if (house_nr.getValue() != null && street_address.getValue() != null)
                		label = house_nr.getValue().getDisplayText() + " " + street_address.getValue().getDisplayText();
                }
                
                String saveName = label;
